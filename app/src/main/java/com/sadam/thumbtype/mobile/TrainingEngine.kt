...
object TrainingEngine {
    ...
    fun calculateResult(
        ...
        val minutes = safeDuration / 60000.0
        ...
        val rawWpm = ((correctChars / 5.0) / minutes).roundToInt().coerceAtLeast(0)
        val errorPenalty = if (minutes > 0) (mistakes / minutes).roundToInt() else 0
        val netWpm = (rawWpm - errorPenalty).coerceAtLeast(0)
        val accuracy = ((correctChars.toDouble() / total) * 100).roundToInt().coerceIn(0, 100)
        val intervals = correctEvents.drop(1).map { it.elapsedFromPreviousMs }
        val rhythm = rhythmScore(intervals)
        val consistency = consistencyScore(intervals)
        val techniqueEvents = correctEvents.filter { it.recommended != ThumbSide.FLEX && it.touchSide != ThumbSide.FLEX }
        val technique = if (techniqueEvents.isEmpty()) 100 else (techniqueEvents.count { it.recommended == it.touchSide } * 100.0 / techniqueEvents.size).roundToInt().coerceIn(0, 100)
        val left = correctEvents.count { it.touchSide == ThumbSide.LEFT }
        val right = correctEvents.count { it.touchSide == ThumbSide.RIGHT }
        val sideTotal = (left + right).coerceAtLeast(1)
        val balance = (100.0 - abs(left - right) * 100.0 / sideTotal).roundToInt().coerceIn(0, 100)
        val speedScore = (netWpm * 100.0 / targetWpm.coerceAtLeast(20)).roundToInt().coerceIn(0, 100)
        val overall = (speedScore * .26 + accuracy * .29 + rhythm * .16 + consistency * .11 + technique * .13 + balance * .05).roundToInt().coerceIn(0, 100)

        val keyMap = linkedMapOf<Char, KeyAggregate>()
        correctEvents.forEach { event ->
            val key = event.expected.lowercaseChar()
            val old = keyMap[key] ?: KeyAggregate()
            keyMap[key] = old.copy(presses = old.presses + 1, totalReactionMs = old.totalReactionMs + event.elapsedFromPreviousMs)
        }
        events.filter { !it.correct }.forEach { event ->
            val key = event.expected.lowercaseChar()
            val old = keyMap[key] ?: KeyAggregate()
            keyMap[key] = old.copy(errors = old.errors + 1)
        }

        val transitionMap = linkedMapOf<String, TransitionAggregate>()
        correctEvents.zipWithNext().forEach { (a, b) ->
            val pair = "${a.expected.lowercaseChar()}${b.expected.lowercaseChar()}"
            val old = transitionMap[pair] ?: TransitionAggregate()
            transitionMap[pair] = old.copy(count = old.count + 1, totalMs = old.totalMs + b.elapsedFromPreviousMs)
        }

        val weakKey = keyMap.entries.filter { it.key.isLetterOrDigit() }.maxByOrNull { it.value.errors * 700 + it.value.averageReactionMs }?.key
        val weakTransition = transitionMap.entries.maxByOrNull { it.value.errors * 800 + it.value.averageMs }?.key

        return SessionResult(
            title, rawWpm, netWpm, accuracy, rhythm, consistency, technique, balance,
            overall * 10, mistakes, mistakes, correctChars, safeDuration, left, right,
            weakKey, weakTransition, keyMap, transitionMap
        )
    }
    ...
    fun generateWeakDrill(
        ...
        val weakKeys = keyStats.entries
            .sortedByDescending { it.value.errors * 600 + it.value.averageReactionMs }
            .map { it.key }
            .filter { it.isLetter() }
            .take(4)
        ...
        val chosen = bank.filter { word -> weakKeys.any { it in word } || weakPairs.any { it in word } }.take(14)
